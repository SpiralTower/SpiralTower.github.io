...
function show(name) {
  ...
  if (obj) {
    if (blockshow) obj.style.display = "block";else obj.style.visibility = "visible";
  }
}
...
function hide(name) {
  ...
  if (obj) {
    if (blockshow) obj.style.display = "none";else obj.style.visibility = "hidden";
  }
}
...
function addLink(name, linkname) {
  ...
  if (obj) {
    obj.appendChild(document.createElement("<a name=\"" + linkname + "\">"));
  }
} //Basic Vaiable
...
var src_Textbox_Selection = sizestr + "/" + folder[5] + "Textbox_Selection" + appendix; //var src_Textbox_Jump=		sizestr+"/"+folder[5]+"Textbox_Jump"			+appendix;
...
document.body.background = src_background; //Object Definition
//Point
...
function FooterBase(x, y, height, page) {
  ...
  if (page != null) {
    if (bw.ie6) this.HTML += "<div style=\"margin-top:" + COMMENTBGMARGINTOP + "px;\">" + getImgStr(src_footer_page[Math.floor(this.page / 10)], NUMBERWIDTH, NUMBERHEIGHT, "pgImg1") + "" + getImgStr(src_footer_page[Math.floor(this.page % 10)], NUMBERWIDTH, NUMBERHEIGHT, "pgImg2") + "</div>";else this.HTML += "<div style=\"margin-top:" + COMMENTBGMARGINTOP + "px;\"><img id=\"pgImg1\" ondblclick=\"jumpPage()\" onmousewheel=\"mouseWheel(true);\" onmouseover=\"cancelWheel(true);cancelRightClick(true);\" onmouseout=\"cancelWheel(false);cancelRightClick(false);\" src=\"" + src_footer_page[Math.floor(this.page / 10)] + "\"><img id=\"pgImg2\" ondblclick=\"jumpPage()\" onmousewheel=\"mouseWheel(false);\" onmouseover=\"cancelWheel(true);cancelRightClick(true);\" onmouseout=\"cancelWheel(false);cancelRightClick(false);\" src=\"" + src_footer_page[Math.floor(this.page % 10)] + "\"></div>";
  }
...
function CommentBase(x, y, text, num, summary, hint, setsumei, option, icon, iconillu, titlecss, contentcss, optioncss, eventcss) {
  ...
  if (this.option != "") {
    ...
    for (i = 0; i < tempString.length && i < 3; i++) {
      if (tempString[i].charAt(0) == "-") this.HTML += "<td width=\"" + COMMENTBARWIDTH + "\" height=\"" + COMMENTBARHEIGHT + "\" style=\"" + getBgStr(src_commentbase_option_B) + "\" valign=\"bottom\" align=\"center\"><div class=\"" + optioncss + sizestr + "\">" + tempString[i].substring(1) + "</div></td>";else this.HTML += "<td width=\"" + COMMENTBARWIDTH + "\" height=\"" + COMMENTBARHEIGHT + "\" style=\"" + getBgStr(src_commentbase_option_A) + "\" valign=\"bottom\" align=\"center\"><div class=\"" + optioncss + sizestr + "\">" + tempString[i].substring(1) + "</div></td>";
    }
  ...
  this.HTML += "</div>";
  this.HTML += "<script>";
  this.HTML += "tempHeight=getObjectRef(\"" + this.ref + "Face\").clientHeight;"; //if(bw.safari&&this.icon!="")this.HTML+="tempHeight+=40;";
...
function vmlline(x, y, l, h) {
  ...
  for (i = 0; i < this.times; i++) {
    ...
    } else if (this.type == "hv") this.HTML += "<div id=\"" + this.ref + "Face\" style=\" width:" + LINESIZE + "px; height:" + (LINEBASEHEIGHT / 2 + 2 * outersize) + "px;" + getBgStr(this.bgsrc) + " margin-top:-" + 2 * outersize + "px;\"></div>";
  }
...
function Linkline() {
  ...
  if (args[0].upcenter != null) this.Points[this.Points.length] = args[0].upcenter;else this.Points[this.Points.length] = args[0];

  for (i = 1; i < args.length - 1; i++) this.Points[this.Points.length] = args[i];

  if (args[args.length - 1].lowercenter != null) this.Points[this.Points.length] = args[args.length - 1].lowercenter;else this.Points[this.Points.length] = args[args.length - 1];
...
function Textbox_Date(x, y, text, type) {
  if (type == "top") tempObj = new ButtonBase(x + (BUTTONWIDTH - DATEWIDTH) / 2, y - (DATEHEIGHT - BUTTONHEIGHT), "date", text, "date");else tempObj = new ButtonBase(x + (BUTTONWIDTH - DATEWIDTH) / 2, y, "date", text, "date");
  return tempObj;
}
...
var pic_src = new Array(src_background, num0_s, num1_s, num1_s, num2_s, num3_s, num4_s, num5_s, num6_s, num7_s, num8_s, num9_s, num0, num1, num1, num2, num3, num4, num5, num6, num7, num8, num9, src_Textbox_Block, src_Textbox_Condition, src_Textbox_Selection,
/*src_Textbox_Jump,*/
src_Textbox_Option, src_Textbox_Date, src_bubble_enter, src_bubble_exit, src_bubble_untrigger, src_route_public, src_route_misae, src_route_tomoyo, src_route_yukine, src_route_fujibayashi, src_route_kappei, src_route_sunohara, src_route_kotomi, src_route_fuuko, src_route_nagisa, src_route_gunfight, src_header_bg, src_footer_top, src_footer_mid, src_footer_bottom, src_commentbase_check, src_commentbase_title, src_commentbase_base, src_commentbase_comment, src_commentbase_hint, src_commentbase_option_A, src_commentbase_option_B, src_commentbase_event, jien1, jien2, jien3, jien4, jien5, jien6, jien7, jien8, jiun1, jiun2, jiun3, jiun4, jiun5, jiun6, jiun7, jiun8, src_dot_v, src_dot_hv, src_dot_h, src_board_top, src_board_mid, src_board_bottom);
var checkPicObj = new Array();
var picInited = false;
var checkTimeout;
...
function checkPic(i) {
  ...
  if (!checkPicObj[i].complete) checkTimeout = window.setTimeout("checkPic(" + i + "," + pic_src.length + ")", 10);else {
    picInited = false;
...
document.oncontextmenu = rightClick; //Select Page
...
function getPage(ori, delta) {
  if (Math.abs(delta) > 1) {
    if (ori + delta == totalPage + 10) return 0;else if (ori + delta == 0 - 10) return totalPage;else if (ori + delta > totalPage) return totalPage;else if (ori + delta < 0) return 0;else return ori + delta;
  } else {
    if (ori + delta == totalPage + 1) return 0;else if (ori + delta == 0 - 1) return totalPage;else return ori + delta;
  }
}
...
function cancelWheel(type) {
  if (type) document.onmousewheel = function () {
    return false;
  };else document.onmousewheel = null;
}

function cancelRightClick(type) {
  if (type) document.oncontextmenu = function () {
    selectPage = hdft["footer"].page;
    setPage(selectPage, "jump");
    return false;
...
function blinkStart(obj) {
  ...
  if (!blinkOriFunc) blinkOriFunc = function () {
    return false;
  };
...
function blink(obj, toShow) {
  ...
  if (toShow) obj.show();else obj.hide();
...
function getBgStr(imgSrc) {
  if (bw.ie6) return "filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, sizingMethod=crop, src=" + imgSrc + ");";else return "background:url(" + imgSrc + ");";
}

function getImgStr(imgSrc, width, height, id) {
  if (bw.ie6) return "<table border=0 cellpadding=0 cellspacing=0 style=\"display:inline;\"><tr><td><div" + (id ? " id=\"" + id + "\"" : "") + " style=\"width:" + width + "px;height:" + height + "px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, sizingMethod=scale, src=" + imgSrc + ");\"></div></td></tr></table>";else return "<img" + (id ? " id=\"" + id + "\"" : "") + " src=\"" + imgSrc + "\" width=\"" + width + "\" height=\"" + height + "\">";
}// JavaScript Document
